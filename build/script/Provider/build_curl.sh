...
if [ $# = 0 ]; then
    if [ "$BUILD_FOR_ARM" = "true" ];then
        CFLAGS="-fstack-protector-all -Wl,-z,relro,-z,now" ./configure --prefix=/usr/local/curl --with-ssl=/usr/local/openssl --with-ssl-backend=openssl --host=aarch64-linux-gnu --build=aarch64-gnu-linux --with-gnu-ld
        lib_out="aarch64"
    elif [ "$BUILD_FOR_NDK_AARCH64" = "true" ];then
        CFLAGS="-fstack-protector-all" LDFLAGS="-Wl,-z,relro,-z,now" ./configure --prefix=/usr/local/curl --with-ssl=/usr/local/openssl --with-ssl-backend=openssl --host=aarch64-linux-android CC=aarch64-linux-android-gcc
        lib_out="ndk-aarch64"
    elif [ "$BUILD_FOR_MACOS" = "true" ];then
        CFLAGS="-fstack-protector-all" ./configure --prefix=/usr/local/curl --with-ssl=/usr/local/openssl --with-ssl-backend=openssl
        lib_out="macos"
    else
        # 确保使用 --with-ssl 指向 Tongsuo 安装路径
        CFLAGS="-fstack-protector-all -Wl,-z,relro,-z,now" ./configure --prefix=/usr/local/curl --with-ssl=/usr/local/openssl --with-ssl-backend=openssl #--with-nghttp2=/usr/local/nghttp2
        lib_out="linux_x64"
	fi
